...
#countries=["Austria","Belgium"] for the different meeting distributions experiment
countries=["Austria","France"]
parser = argparse.ArgumentParser()
parser.add_argument('--testdays', type=int, default=1, help='Optional testdays value (default: 10)')
args = parser.parse_args()
testdays=args.testdays #how many days in the future we are predicting
pop=[8677088,64837414,11623928] # array of the population of each locale (third is Belgium)
#population from each group having a distinct meeting distribution
pop1=[0.2*8677088,0.2*64837414,0.2*11623928]
pop2=[0.6*8677088,0.6*64837414,0.6*11623928]
pop3=[0.2*8677088,0.2*64837414,0.2*11623928]
days=101 # total number of days of the simulation
cnum=2 # number of locales
...
def sim(p1,p2):
	...
	for i in range(cnum):
		for t in range(days-testdays-1,days-testdays):
			res[i][t]=(simdata[i][t][2][0]-cdata[i][t][2][0])/cdata[i][t][2][0]
...
def main():
	...
	print(f"Start EpiInfer Multilocale output for testdays={testdays}")
	print(sum(msels)/len(msels))
	print(f"End EpiInfer Multilocale output for testdays={testdays}")
